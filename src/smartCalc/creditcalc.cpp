...
void CreditCalc::calc_credit() {
  ...
  if (!(sum.isEmpty() || procent.isEmpty() || deadline.isEmpty())) {
    ...
    if (button == ui->pushButton_DIFF) {
      payment =
          calculateDifferential(sum_number, procent_number, deadline_number);
      mounth_pay = QString("%L1").arg(payment, 0, 'f', 2);
      total = calculateTotal(sum_number, procent_number, deadline_number);
    }
    if (button == ui->pushButton_ANN) {
      payment = calculateAnnuity(sum_number, procent_number, deadline_number);
      mounth_pay = QString("%L1").arg(payment, 0, 'f', 2);
