...
START_TEST(Smart_calc_31) {
  int error = popvalue(NULL, NULL);
  ck_assert_int_eq(error, -1);
}
END_TEST

START_TEST(Smart_calc_32) {
  NValue *ValueNode = NULL;
  long double value;
  int error = popvalue(&ValueNode, &value);
  ck_assert_int_eq(error, -1);
}
END_TEST
...
START_TEST(Smart_calc_X1) {
  ...
  long double x = 5.5;
  int error = SmartCalc_Graph(input_str, &result, x);
...
START_TEST(creditcalc_1) {
  double result = calculateAnnuity(100000, 5, 12);
...
START_TEST(creditcalc_2) {
  double result = calculateAnnuity(100000, 12, 5);
...
START_TEST(creditcalc_3) {
  double result = calculateDifferential(113410, 24, 7);
...
START_TEST(creditcalc_4) {
  double result = calculateTotal(100000, 5, 12);
...
START_TEST(creditcalc_5) {
  double result = calculateTotal(113410, 24, 7);
